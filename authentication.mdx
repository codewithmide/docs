...
| `X-Idempotency-Key` | Unique request identifier | Required when creating a checkout or master-wallet payout |
...
Creating a checkout or master-wallet payout requires an idempotency key. Send either `X-Idempotency-Key` (recommended) or the backward-compatible `Idempotency-Key` header. If both headers are sent, their values must match exactly or the API returns `400 Bad Request`.

Reusing the same key with the same request payload returns the original resource without creating or debiting another one. Reusing it with a different payload returns `409 Conflict`. Idempotency keys can contain up to 255 characters and are scoped to your merchant account.
