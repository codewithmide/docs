...
```bash
curl -X GET https://api-production.fossapay.com/api/v1/customers/cust_123456789/tier-status \
  -H "x-api-key: YOUR_API_KEY"
```
...
```javascript
const response = await fetch('https://api-production.fossapay.com/v1/wallets/crypto/create', {
  method: 'POST',
  headers: {
    'x-api-key': 'YOUR_API_KEY',
    'Content-Type': 'application/json'
