...
Transfer funds instantly and free of charge between two FossaPay customers under the same business. Both the sending and receiving account numbers must belong to FossaPay NGN wallets created under your business:
...
<Note>
  Wallet-to-wallet transfers are free and atomic - either both the debit and credit succeed, or neither happens. Use an inter-bank transfer when the recipient uses an external bank.
</Note>
...
### Neobank Wallet
```javascript
// Create NGN wallet when user signs up
...
This free flow applies only when both users are customers under your business and both accounts are FossaPay NGN wallets.
...
<CardGroup cols={2}>
  <Card color="#0080FF" title="Create NGN Wallet" icon="code" href="/api-reference/virtual-account/create-wallet">
