...
|-------|-------------|---------------|
| `checkout.completed` | Checkout payment completed | After the exact payable amount is confirmed |
| `checkout.failed` | Checkout payment failed | When processing fails, including amount mismatch |
| `checkout.expired` | Checkout expired | When a pending checkout passes or reaches its expiry |
| `checkout.reversed` | Checkout payment reversed | After checkout funds are returned to the sender |

### Master Wallet Payout Events

| Event | Description | When it fires |
|-------|-------------|---------------|
| `payout.completed` | Master-wallet payout completed | When the beneficiary payout succeeds |
| `payout.failed` | Master-wallet payout failed | When the payout fails without a confirmed full reversal |
| `payout.reversed` | Master-wallet payout reversed | When the payout funds are confirmed returned |
...
app.post('/webhooks/fossapay', async (req, res) => {
  ...
  if (!verifySignature(req.body.data, signature, secret)) {
...
def process_webhook(payload):
    ...
    if event == 'deposit.completed':
        ...
        handle_payout_terminal_state(event, data)
```
...
curl -X POST https://api-production.fossapay.com/api/v1/webhooks/simulate \
  -H "Authorization: Bearer fk_production_DTKUG..." \
