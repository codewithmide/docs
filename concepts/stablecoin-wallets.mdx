...
Customer wallets can be retrieved by FossaPay wallet ID, customer ID, or full blockchain address. The master endpoint returns merchant-owned Solana addresses and provider-reported SOL, USDC, and USDT balances. Unsupported tokens are recorded in transaction history, not these standard balance responses.
