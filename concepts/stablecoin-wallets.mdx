...
Solana wallets assign an address to an individual customer. FossaPay reports balances for native SOL, USDC, and USDT. Only USDC and USDT can be sent through the current crypto transfer API; SOL transfers are not yet available through FossaPay.
...
| `sol` | Native SOL deposits and balance reporting; transfer API unavailable |
...
- [ ] Use `solana` for the wallet network; use USDC or USDT for checkout, payment links, and transfers.
