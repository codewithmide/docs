...
Wallet creation returns `data.id` (FossaPay's wallet UUID), `data.walletId` (the underlying provider wallet identifier), the Solana `data.address`, and the network. Persist the complete response with the customer UUID. Use `data.id` for the wallet-by-ID and wallet-transactions endpoints; use `data.address` for the wallet-by-address endpoint. Neither the Solana address nor the provider `walletId` works as the `{walletId}` path parameter.
...
| Sign with master wallet | `POST /api/v1/wallets/crypto/master/transactions/sign-and-broadcast` | [Master wallet signing](/api-reference/crypto-wallets/sign-master-transaction) |
| Sign with customer wallet | `POST /api/v1/wallets/crypto/customer/{customerId}/transactions/sign-and-broadcast` | [Customer wallet signing](/api-reference/crypto-wallets/sign-customer-transaction) |
...
For custom Solana program interactions, follow the [Solana Transaction Signing guide](/guides/solana-transaction-signing). Signing supports arbitrary validated Solana versioned transactions; it does not expand the assets supported by FossaPay transfers, checkouts, or payment links.
