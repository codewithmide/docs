...
This guide walks you through accepting payments using FossaPay's NGN Wallet API. You'll learn how to create NGN wallets, receive payments, handle webhooks, and reconcile transactions.

## Quick Start

<Steps>
  <Step title="Create an NGN Wallet">
...
```bash
# Create NGN wallet for customer if not exists
curl -X POST https://api-production.fossapay.com/api/v1/wallets/fiat/create \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
app.post('/webhooks/fossapay', async (req, res) => {
  // 1. Verify signature
  if (!verifySignature(req.body.data, req.headers['x-fossapay-signature'])) {
...
```json
{
  ...
  "data": {
    ...
    "customerId": "cus_123",
    "createdAt": "2024-01-15T10:30:40Z"
  }
}
```
...
<AccordionGroup>
  ...
  <Accordion title="Handle Webhooks Idempotently">
    Always check for duplicates using `transactionId` or `eventId`.
  </Accordion>
...
### Test with a Small Live Deposit

FossaPay operates in production only, so test the collection flow with a small live transfer that you control.

1. Create a virtual account for the customer.
2. Send a small transfer from a real bank account you control.
3. Confirm that the deposit webhook fires and the balance updates.
4. Verify your reconciliation logic against the live transaction record.

## Next Steps

<CardGroup cols={2}>
  <Card color="#0080FF" title="Create NGN Wallet API" icon="code" href="/api-reference/virtual-account/create-wallet">
    API reference for creating NGN wallets
  </Card>
  <Card color="#0080FF" title="Webhooks Guide" icon="webhook" href="/concepts/webhooks">
    Learn more about handling webhooks
  </Card>
  <Card color="#0080FF" title="Wallets Concept" icon="building-columns" href="/concepts/wallets">
